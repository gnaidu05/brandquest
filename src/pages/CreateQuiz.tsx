  ...
  const validate = (): string[] => {
    ...
    questions.forEach((q, i) => {
      ...
      q.options.forEach((opt, j) => {
        if (!opt.trim())
          errs.push(`Question ${i + 1}, Option ${j + 1}: is empty`);
      });
    });
    return errs;
  };
      ...
      <div className="max-w-5xl mx-auto px-4 pt-24 pb-12">
        ...
        >
          <button
            ...
            className="text-white/50 hover:text-white transition-colors mb-4 flex items-center gap-2"
          >
            ← Back to Dashboard
          </button>
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            >
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  onClick={addQuestion}
                  className="text-sm px-3 py-1 rounded-lg bg-primary/30 hover:bg-primary/50 transition-colors"
                >
                  + Add
                </button>
              </div>
                ...
                {questions.map((q, i) => (
                  <button
                    ...
                    className={`w-full text-left p-3 rounded-xl transition-all flex items-center gap-3 ${
                      activeQuestion === i
                        ? "bg-primary/30 ring-1 ring-primary-light"
                  ...
                  >
                    ...
                    {questions.length > 1 && (
                      <span
                        ...
                        className="text-white/30 hover:text-kahoot-red transition-colors text-xs"
                      >
                        ✕
                      </span>
                    )}
          ...
          <div className="lg:col-span-2">
            <AnimatePresence mode="wait">
              ...
              >
                <div className="flex items-center justify-between mb-6">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        updateQuestion(activeQuestion, "timeLimit", Number(e.target.value))
                      }
                    ...
                    <div key={i}>
                      <label className="text-xs text-white/40 mb-1 block">
                        ...
                        {currentQ.correctIndex === i && (
                          <span className="text-kahoot-green ml-1">
                            (correct)
                          </span>
                        )}
                      </label>
                      <AnswerButton
                        text={opt || "Enter answer..."}
                        index={i}
                        variant="compact"
                        onClick={() =>
                          updateQuestion(activeQuestion, "correctIndex", i)
                        }
                      />
                      <input
                        type="text"
                ...
                <p className="text-xs text-white/30 mt-4 text-center">
                  Click an answer button to mark it as correct (green outline)
                </p>
                ...
                <div className="flex items-center justify-between mt-6 pt-6 border-t border-white/10">
                  <button
                    ...
                    className="px-4 py-2 rounded-xl bg-white/10 hover:bg-white/20 transition-colors disabled:opacity-30"
                  >
                    ← Previous
                  </button>
                  ...
                  {activeQuestion < questions.length - 1 ? (
                    <button
                      onClick={() =>
                        setActiveQuestion(Math.min(questions.length - 1, activeQuestion + 1))
                      }
                    ...
                    <button
                      onClick={addQuestion}
                      className="px-4 py-2 rounded-xl bg-primary/30 hover:bg-primary/50 transition-colors"
                    >
                      + Add Question
                    </button>
                  )}
            ...
            {errors.length > 0 && (
              <motion.div
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
                className="mt-4 bg-kahoot-red/20 border border-kahoot-red/30 rounded-xl p-4"
              >
                {errors.map((err, i) => (
                  <p key={i} className="text-sm text-kahoot-red-light">
                    {err}
                  </p>
                ))}
              </motion.div>
            ...
            <motion.button
              ...
              onClick={handleSave}
              disabled={saving}
              className="w-full mt-6 px-6 py-4 rounded-2xl kahoot-gradient text-lg font-bold disabled:opacity-50 disabled:cursor-not-allowed transition-opacity"
            >
